import { execProbeCommand } from "./executor.js";
...
export async function probeExec(binary: string, args: string[]) {
  return execProbeCommand(binary, { args, timeout_ms: PROBE_TIMEOUT_MS });
}
...
export const DEFAULT_GIT_IDENTITY: Readonly<GitIdentityProbe> = Object.freeze({
  configured: false,
  userName: null,
  userEmail: null,
});

export const DEFAULT_GIT_SIGNING: Readonly<GitSigningProbe> = Object.freeze({
  gpgFormat: null,
  commitGpgsign: false,
  signingKey: null,
  signingKeySource: null,
  signingTest: Object.freeze({ attempted: false, success: false, error: null }),
});
